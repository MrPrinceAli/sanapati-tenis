...
// Pengunjung tanpa login boleh menulis skor/nama, jadi dibatasi lajunya per alamat IP (di memori proses).
...
export async function addPlayers(_: FormState, form: FormData): Promise<FormState> {
  ...
  const current = getPlayers(ctx.session.id).filter((p) => p.active).length;
  if (current + names.length > LIMITS.players) return { error: t.errors.mmTooManyPlayers };

  const insert = db.prepare("INSERT INTO mm_players (session_id, name, gender) VALUES (?,?,?)");
  db.transaction(() => names.forEach((n) => insert.run(ctx.session.id, n, gender)))();
...
export async function renamePlayer(_: FormState, form: FormData): Promise<FormState> {
  ...
  const gender = form.get("gender") === "F" ? "F" : "M";
  db.prepare("UPDATE mm_players SET name = ?, gender = ? WHERE id = ? AND session_id = ?").run(
    name,
    gender,
    Number(form.get("playerId")),
    ctx.session.id
  );
  refresh(ctx.session.id);
  return { ok: t.ok.sessionSaved };
...
export async function removePlayer(_: FormState, form: FormData): Promise<FormState> {
  ...
  const involved = getMatches(session.id).filter((m) => [m.a1, m.a2, m.b1, m.b2].includes(playerId));
  if (involved.length === 0) {
    db.prepare("DELETE FROM mm_players WHERE id = ? AND session_id = ?").run(playerId, session.id);
    ...
    db.transaction(() => {
      db.prepare("UPDATE mm_players SET active = 0 WHERE id = ? AND session_id = ?").run(playerId, session.id);
      const del = db.prepare("DELETE FROM mm_matches WHERE id = ?");
      involved.filter((m) => !isScored(m)).forEach((m) => del.run(m.id));
    })();
  }
  refresh(session.id);
  return { ok: "" };
...
const insertMatch = () =>
  db.prepare(
    "INSERT INTO mm_matches (session_id, round, slot, a1, a2, b1, b2, is_bye) VALUES (@session, @round, @slot, @a1, @a2, @b1, @b2, @bye)"
  );
const cols = (s: Sides) => ({ a1: s.a[0] ?? null, a2: s.a[1] ?? null, b1: s.b[0] ?? null, b2: s.b[1] ?? null });
...
export async function generateMatches(_: FormState, form: FormData): Promise<FormState> {
  ...
  if ("error" in ctx) return ctx;
  const { session } = ctx;
  const players = getPlayers(session.id).filter((p) => p.active);
  ...
  if (session.mode === "tournament") {
    ...
    db.transaction(() => {
      db.prepare("DELETE FROM mm_matches WHERE session_id = ?").run(session.id);
      const ins = insertMatch();
      for (const m of bracket) ins.run({ session: session.id, round: m.round, slot: m.slot, ...cols(m), bye: m.isBye ? 1 : 0 });
    })();
  } else {
    ...
    db.transaction(() => {
      if (!addOne) {
        const del = db.prepare("DELETE FROM mm_matches WHERE id = ?");
        matches.filter((m) => !isScored(m)).forEach((m) => del.run(m.id));
      }
      const ins = insertMatch();
      for (const r of planned) r.matches.forEach((m, slot) => ins.run({ session: session.id, round: r.round, slot, ...cols(m), bye: 0 }));
    })();
  }
  db.prepare("UPDATE mm_sessions SET finished = 0 WHERE id = ?").run(session.id);
  refresh(session.id);
  return { ok: "" };
}

// Turnamen: pemenang (atau kekosongan, saat skor dihapus) diteruskan ke slot babak berikutnya.
function advance(session: MMSession, match: MMMatch, winner: number[] | null) {
  const next = db
    .prepare("SELECT * FROM mm_matches WHERE session_id = ? AND round = ? AND slot = ?")
    .get(session.id, match.round + 1, match.slot >> 1) as MMMatch | undefined;
  if (!next) return;
  const side = match.slot % 2 === 0 ? "a" : "b";
  db.prepare(`UPDATE mm_matches SET ${side}1 = ?, ${side}2 = ? WHERE id = ?`).run(winner?.[0] ?? null, winner?.[1] ?? null, next.id);
}
...
export async function saveScore(_: FormState, form: FormData): Promise<FormState> {
  ...
  const { session } = ctx;
  const match = db
    .prepare("SELECT * FROM mm_matches WHERE id = ? AND session_id = ?")
    .get(Number(form.get("matchId")), session.id) as MMMatch | undefined;
  ...
  db.transaction(() => {
    db.prepare("UPDATE mm_matches SET score_a = ?, score_b = ? WHERE id = ?").run(clear ? null : a, clear ? null : b, match.id);
    if (session.mode === "tournament") {
      const s = sidesOf(match);
      advance(session, match, clear ? null : a > b ? s.a : s.b);
    }
  })();
  refresh(session.id);
...
export async function toggleFinished(form: FormData) {
  ...
  db.prepare("UPDATE mm_sessions SET finished = ? WHERE id = ?").run(ctx.session.finished ? 0 : 1, ctx.session.id);
  refresh(ctx.session.id);
}
...
export async function deleteSession(form: FormData) {
  ...
  const ctx = await manageable(form, t);
  if ("error" in ctx) return;
  // mm_players dan mm_matches ikut terhapus lewat ON DELETE CASCADE.
  db.prepare("DELETE FROM mm_matches WHERE session_id = ?").run(ctx.session.id);
  db.prepare("DELETE FROM mm_sessions WHERE id = ?").run(ctx.session.id);
  revalidatePath("/mabar");
  redirect("/mabar");
}
